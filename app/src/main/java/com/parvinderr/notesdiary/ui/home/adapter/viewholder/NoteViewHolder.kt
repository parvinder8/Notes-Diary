...
class NoteViewHolder(private val binding: ItemLayoutNoteBinding) :
    ...
    fun setViews(item: Note) {
        binding.apply {
            if (item.noteTitle.isEmpty()) tvNoteTitle.gone()
            else tvNoteTitle.visible()
            tvNoteTitle.text = item.noteTitle.substring(
                0, if (item.noteTitle.length > 40) {
                    40
                } else item.noteTitle.length
            )
            tvNoteContent.text = item.noteContent.substring(
                0, if (item.noteContent.length > 150) {
                    150
                } else item.noteContent.length
            )
